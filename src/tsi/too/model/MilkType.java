...
public enum MilkType {
    ...
    FRESH(Constants.FRESH),
    ICE_COLD(Constants.ICE_COLD);

    private static Map<Integer, MilkType> mappedValues;
    private String description;

    MilkType(@NotNull String description) {
    ...
    @NotNull
    public static MilkType from(int ordinal) throws IllegalArgumentException {
        mapValues();

        MilkType type = mappedValues.get(ordinal);
    ...
    private static void mapValues() {
        if (mappedValues == null)
            mappedValues = Arrays.stream(values())
